...
public abstract class RubyBaseNode extends Node {
    ...
    protected final RubyString createString(TruffleString.FromByteArrayNode fromByteArrayNode, byte[] bytes,
        ...
        var tstring = fromByteArrayNode.execute(bytes, 0, bytes.length, encoding.tencoding, false);
        return createString(tstring, encoding);
    }

    protected final RubyString createSubString(TruffleString.SubstringByteIndexNode substringByteIndexNode,
            RubyStringLibrary strings, Object source, int byteOffset, int byteLength) {
        return createSubString(substringByteIndexNode, strings.getTString(source), strings.getEncoding(source),
                byteOffset, byteLength);
    }

    protected final RubyString createSubString(TruffleString.SubstringByteIndexNode substringByteIndexNode,
            AbstractTruffleString tstring, RubyEncoding encoding, int byteOffset, int byteLength) {
        final TruffleString substring = substringByteIndexNode.execute(tstring, byteOffset, byteLength,
                encoding.tencoding, true);
