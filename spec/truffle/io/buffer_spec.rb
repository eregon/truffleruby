...
describe "IO buffers" do
  ...
  it "allocate buffer space and return a non-null pointer" do
    ptr = Primitive.io_fiber_buffer_allocate(1)
    begin
  ...
  it "allocate multiple buffers and returns non-null pointers" do
    ptr1 = Primitive.io_fiber_buffer_allocate(1)
    ptr2 = Primitive.io_fiber_buffer_allocate(1)
    begin
      ptr1.null?.should == false
      ptr2.null?.should == false
      @mem_check.call(ptr1, ptr2)
    ensure
      Primitive.io_fiber_buffer_free(ptr2)
      Primitive.io_fiber_buffer_free(ptr1)
    end
  end

  it "allocate multiple buffers and returns non-null pointers for large buffer sizes" do
    ptr1 = Primitive.io_fiber_buffer_allocate(4096 * 4)
    ptr2 = Primitive.io_fiber_buffer_allocate(4096 * 4)
    begin
      ptr1.null?.should == false
      ptr2.null?.should == false
      @mem_check.call(ptr1, ptr2)
